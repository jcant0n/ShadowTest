...
{
	float4x4 ShadowViewProjectionArray[16] : packoffset(c0.x); [ShadowViewProjectionBuffer]
};
...
Texture2DArray ShadowMapArray2D			: register(t10); [ShadowMapArray]
TextureCubeArray ShadowMapArrayCube		: register(t11); [ShadowMapArrayCube]
...
inline float SampleShadowMap(in float2 base_uv, in float u, in float v, in float2 shadowMapSizeInv,
    ...
    return ShadowMapArray2D.SampleCmpLevelZero(ShadowMapSampler, float3(uv, cascadeIdx), depth);
}

float SampleShadowMapPCF(in float3 shadowPosition, in int cascadeIdx, in float shadowBias)
{
    float2 shadowTexcoord = shadowPosition.xy * float2(0.5, -0.5) + float2(0.5, 0.5);
	float lightDepth = shadowPosition.z - shadowBias;
	
#if SHADOWFILTER3 || SHADOWFILTER5 || SHADOWFILTER7
	float2 shadowMapSize;
    float numSlices;
    ShadowMapArray2D.GetDimensions(shadowMapSize.x, shadowMapSize.y, numSlices);
    ...
    float sum = 0;
	    ...
	    sum += uw0 * vw0 * SampleShadowMap(base_uv, u0, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw1 * vw0 * SampleShadowMap(base_uv, u1, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw2 * vw0 * SampleShadowMap(base_uv, u2, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
	
	    sum += uw0 * vw1 * SampleShadowMap(base_uv, u0, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw1 * vw1 * SampleShadowMap(base_uv, u1, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw2 * vw1 * SampleShadowMap(base_uv, u2, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
	
	    sum += uw0 * vw2 * SampleShadowMap(base_uv, u0, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw1 * vw2 * SampleShadowMap(base_uv, u1, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
	    sum += uw2 * vw2 * SampleShadowMap(base_uv, u2, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
		...
		sum += uw1 * vw0 * SampleShadowMap(base_uv, u1, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw2 * vw0 * SampleShadowMap(base_uv, u2, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw3 * vw0 * SampleShadowMap(base_uv, u3, v0, shadowMapSizeInv, cascadeIdx, lightDepth);
		
		sum += uw0 * vw1 * SampleShadowMap(base_uv, u0, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw1 * vw1 * SampleShadowMap(base_uv, u1, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw2 * vw1 * SampleShadowMap(base_uv, u2, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw3 * vw1 * SampleShadowMap(base_uv, u3, v1, shadowMapSizeInv, cascadeIdx, lightDepth);
		
		sum += uw0 * vw2 * SampleShadowMap(base_uv, u0, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw1 * vw2 * SampleShadowMap(base_uv, u1, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw2 * vw2 * SampleShadowMap(base_uv, u2, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw3 * vw2 * SampleShadowMap(base_uv, u3, v2, shadowMapSizeInv, cascadeIdx, lightDepth);
		
		sum += uw0 * vw3 * SampleShadowMap(base_uv, u0, v3, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw1 * vw3 * SampleShadowMap(base_uv, u1, v3, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw2 * vw3 * SampleShadowMap(base_uv, u2, v3, shadowMapSizeInv, cascadeIdx, lightDepth);
		sum += uw3 * vw3 * SampleShadowMap(base_uv, u3, v3, shadowMapSizeInv, cascadeIdx, lightDepth);
		
		return sum * 1.0 / 2704;
		
	#endif
#else
	return ShadowMapArray2D.SampleCmpLevelZero(ShadowMapSampler, float3(shadowTexcoord.xy, cascadeIdx), lightDepth);
#endif
}

inline float3 ShadowCascade(in LightProperties lightProperties, in float3 shadowPosition, in int cascadeIdx)
{
	float3 shadow = 0;
...
void SpotLight(const ShadingParams shading, const MaterialInputs material, const PixelParams pixel, const LightProperties lightProperties, inout float3 color)
{
	...
	if (NoL * attenuation > 0)
	{
		...
		if(lightProperties.IsCastingShadow())
		{
			float4 shadowPosition = mul(float4(shading.position, 1), ShadowViewProjectionArray[lightProperties.ShadowMapIndex]);
