...
[directives:ReciverPlaneDepthBias RPDB_OFF RPDB]
...
{
	float4x4 ShadowViewProjectionArray[8] : packoffset(c0.x); [ShadowViewProjectionBuffer]
};
...
float SampleShadowMap(in float2 base_uv, in float u, in float v, in float2 shadowMapSizeInv,
    ...
    return ShadowMapArray2D.SampleCmpLevelZero(ShadowMapSampler, float3(uv, cascadeIdx), z);
}

float SampleShadowMapPCF(in float2 texcoord, in int cascadeIdx, in float lightDepth)
{
    ...
    float2 uv = texcoord.xy * shadowMapSize; // 1 unit - 1 texel
    ...
    float sum = 0;
		...
		return ShadowMapArray2D.SampleCmpLevelZero(ShadowMapSampler, float3(texcoord.xy, cascadeIdx), lightDepth);
...
inline float3 ShadowCascade(in LightProperties lightProperties, in float3 shadowPosition, in int cascadeIdx)
{
	...
	const float lightDepth = shadowPosition.z - lightProperties.ShadowBias;
	float3 shadow = 0;
    ...
    //float shadowDistance = ShadowMapArray2D.Sample(BaseColorSampler, float3(shadowTexcoord, lightProperties.ShadowMapIndex + cascadeIdx)).r;
	//shadow = (shadowDistance < lightDepth) ? 0.0 : 1.0;

	shadow = SampleShadowMapPCF(shadowTexcoord, lightProperties.ShadowMapIndex + cascadeIdx, lightDepth);
